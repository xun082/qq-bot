...
/** 群聊专用：整段消息仅为 /clear（忽略大小写、首尾空白）时清空该用户在本群的上下文，不调用 AI */
function isClearCommand(text: string): boolean {
  return /^\/clear$/i.test(text.trim());
}
...
function start(): void {
  ...
  ws.on("message", (data: WebSocket.RawData) => {
    ...
    if (message_type === "group" && group_id !== undefined) {
      ...
      const historyKey = `group_${group_id}_${user_id}`;

      if (isClearCommand(text)) {
        clearHistory(historyKey);
        console.log(
          `[群聊] ${group_id} | ${nickname}(${user_id}): /clear → 已清空上下文`,
        );
        sendApi(ws, "send_group_msg", {
          group_id,
          message: `[CQ:at,qq=${user_id}] 已清除你在本群与我的对话上下文，可以重新开始。`,
        });
        return;
      }
