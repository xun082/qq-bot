...
function getGroupLogContext(groupId: number): string {
  const log = groupLogs.get(groupId);
  if (!log || log.length === 0) return '';
  const lines = log.map((e) => `${e.nickname}: ${e.text}`).join('\n');
  return `以下是群里最近 ${log.length} 条消息记录（供你参考）：\n${lines}`;
}
...
function pushHistory(key: string, role: ChatMessage['role'], content: string): void {
...
// ── 公司 RWKV 模型 API 调用 ──────────────────────────────────────
async function callAI(historyKey: string, userText: string, groupContext?: string): Promise<string> {
  pushHistory(historyKey, 'user', userText);

  const systemContent = [
    AI_SYSTEM,
    groupContext ?? '',
    '请只回复纯文本，不要使用 Markdown（如 **粗体**、# 标题、``` 代码块等）。',
  ].filter(Boolean).join('\n\n');
  ...
  const raw = (json.choices?.[0]?.message?.content ?? json.choices?.[0]?.text ?? '').trim();

  // 防止模型继续生成下一轮 "User:" 内容
  const stopIdx = raw.search(/\n\s*User:/i);
  const reply = toPlainText(stopIdx !== -1 ? raw.slice(0, stopIdx) : raw) || '（无回复）';
...
function stripCQ(raw: string, selfId?: number): string {
  return raw
    .replace(/\[CQ:at,qq=(\d+)\]/g, (_, qq) => (selfId && qq === String(selfId) ? '' : `@${qq}`))
    .replace(/\[CQ:[^\]]+\]/g, '')
    .trim();
}
...
function sendApi(ws: WebSocket, action: string, params: Record<string, unknown>): void {
...
function start(): void {
  ...
  ws.on('message', (data: WebSocket.RawData) => {
    ...
    const { message_type, raw_message, user_id, group_id, self_id, sender } = payload;
    ...
    if (message_type === 'group' && group_id !== undefined) {
      const plainText = stripCQ(raw_message, self_id);
      ...
      // 把最近 N 条群消息拼进 system prompt，让 AI 有群聊背景
      const groupContext = getGroupLogContext(group_id);
